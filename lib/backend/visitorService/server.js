...
app.get(`${pipe}/getAgreement`, authenticateToken, (req, res) => {
  db.query('SELECT * FROM AGREEMENT WHERE inUse=1', (err, results) => {
      if (err) {
...
app.post(`${pipe}/approvedDocument`, authenticateToken, async (req, res) => {
  try {
    ...
    if(!filename){
      return res.status(404).json({ error: "Failed to move signature file." });
    }
...
// Clear FCM token when not active > 30 day
