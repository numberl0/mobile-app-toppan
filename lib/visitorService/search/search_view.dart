...
class _SearchContentState extends State<SearchContent> with RouteAware {
  ...
  SearchFormController _con = SearchFormController();
  ...
  Widget listRequest() {
    ...
    switch (_con.selectedType) {
      ...
      case RequestType.temporary:
        filteredRequest = _con.filteredTemporaryList;
      default:
        filteredRequest = [];
    }
  ...
  Widget ItemRequest(int index, Map<String, dynamic> entry) {
    ...
    final bool isReturnFactory = entry['guard_ret_at'] != null;
    ...
    (entry['tno_pass']?.toString().isEmpty ?? true)) ...[
  Row(
    children: [
      Expanded(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        if(entry['out_only'] == 0 && entry['guard_status'] == 1) ...[
                          Positioned(
                            bottom: isPhoneScale ? 44 : 47,
                            right: 0,
                            child: InkWell(
                              onTap: isReturnFactory ?  null : () async {
                                await CustomDialog.show(
                                            context: context,
                                            title: 'คำเตือน',
                                            message: 'คุณต้องการบันทึกเวลากลับเอกสารหมายเลข ${entry['sequence_no']} แล้วใช่หรือไม่? การดำเนินการนี้จะไม่สามารถย้อนกลับมาแก้ไขได้',
                                            type: DialogType.info,
                                            onConfirm: () async {
                                              await _con.updateDatetimeReturn(entry);
                                              await preparePage();
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  color: isReturnFactory ? Colors.green : Colors.orange,
                                  borderRadius: BorderRadius.only(
                                    topLeft: Radius.circular(16),
                                  ),
                                  boxShadow: [
                                ...
                                child: Row( // ใช้ Row เพื่อให้ใส่ Icon นำหน้าได้ (ดูเป็นปุ่ม Action มากขึ้น)
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(isReturnFactory ? Icons.check_rounded : Icons.logout_outlined, color: Colors.white, size: _fontSize - 4),
                                    SizedBox(width: 4),
                                    Text(
                                      isReturnFactory ? "ลงเวลาแล้ว" : "ลงเวลากลับ",
                                      style: TextStyle(
                                        color: Colors.white,
                                        fontSize: _fontSize - 4,
                                        fontWeight: FontWeight.bold,
...
Positioned(
  ...
  Widget ContentDetail(Map<String, dynamic> entry) {
    ...
    final timeEnd = endDate != null? DateFormat("HH:mm", "th_TH").format(endDate): '-';
    final timeStart = startDate != null? DateFormat("HH:mm", "th_TH").format(startDate): '-';
    final formattedDateEnd = endDate != null? '${DateFormat("dd MMM yyyy", "th_TH").format(endDate)}    $timeEnd น.' : '-';
    final formattedDateStart = startDate != null? '${DateFormat("dd MMM yyyy", "th_TH").format(startDate)}    $timeStart น.' : '-';


    final fieldSignature = _con.signatureSection[entry['request_type']?.toUpperCase()] ?? [];
      ...
      children: [
        SizedBox(height: 10,),
                if (docType == "VISITOR") ...[
                  ...
                  SizedBox(height: 25),
                  buildLabelValueRow('เวลาเข้า:', formattedDateStart, leftPadding: pddingLabel, labelWidth: spaceLabel, labelBold: true),
                  SizedBox(height: 25),
                  buildLabelValueRow('เวลาออก:', formattedDateEnd, leftPadding: pddingLabel, labelWidth: spaceLabel, labelBold: true),
                  SizedBox(height: 25),
                  ...
                  buildLabelValueRow('ขออนุญาต:', '${_con.eTypeObjectiveMapping[int.tryParse(entry['objective_type'].toString())]}', leftPadding: pddingLabel, labelWidth: spaceLabel, labelBold: true),
                  SizedBox(height: 25),
                  buildLabelValueRow('เวลาออก:', '$formattedDateStart', leftPadding: pddingLabel, labelWidth: spaceLabel, labelBold: true),
                  SizedBox(height: 25),
                  if (entry['out_only'] == 0) ...[
                    buildLabelValueRow('เวลากลับ:', '$formattedDateEnd', leftPadding: pddingLabel, labelWidth: spaceLabel, labelBold: true),
                    SizedBox(height: 25),
                  ],
                  buildLabelValueRow('วัตถุประสงค์:', entry['objective'] ?? '', leftPadding: pddingLabel, labelWidth: spaceLabel, labelBold: true),
                ] else if (docType == "PERMISSION") ...[
          ...
          Container(
              ...
              child: Column(
                children: [
                  ...
                  contentItemDisplay(entry['item_in'], entry['item_out'], docType),
                ],
              ),
            ),
            SizedBox(
