        ...
        switch (docType) {
            case 'visitor':
                [results] = await db.query(
                    ...
                    WHERE DATE(datetime_in) BETWEEN ? AND ?
                    ORDER BY pr.datetime_in ASC`,
                    [sDate, eDate]
                    );
                break;
            case 'employee':
                [results] = await db.query(
                    ...
                    FROM PASS_REQ_E pr
                    LEFT JOIN PASS_FORM pf ON pr.tno_pass = pf.tno_pass
                    WHERE DATE(pr.datetime_out) BETWEEN ? AND ?
                    ORDER BY pr.datetime_out ASC`,
                    [sDate, eDate]
                    );
                break;
            case 'permission':
...
async function generateVisitorLogBook(doc, result) {
    try {
        let pageNo = 1;
        const minRowHeight = 25; // ความสูงขั้นต่ำของแถว
        ...
        for (const item of listDataV) {
            for (const person of item.people) {
                
                // 1. กำหนดโครงสร้างข้อมูลแต่ละ Cell
                const cells = [
                    { label: formatThaiDate(item.datetime_in), width: 50 },
                    ...
                    { label: item.company, width: 100, align: 'left' },
                    { label: item.objective, width: 100, align: 'left' },
                    { label: person.Card_Id, width: 45 },
                    { label: getTime(item.datetime_in), width: 35 },
                    { label: getTime(item.datetime_out), width: 35 },
                ...
                // 2. ตั้งค่า Font ก่อนคำนวณความสูง (สำคัญมาก: Font Size ต้องตรงกับตอนวาดจริง)
                const currentFontSize = 14;
                const lineGapValue = -1;
                doc.fontSize(currentFontSize);

                // 3. คำนวณหาความสูงที่มากที่สุดในแถวนี้ (Dynamic Height)
                let textHeight = Math.max(
                    minRowHeight,
                    ...cells.map(cell => {
                        if (typeof cell.label === 'string' && (cell.label.endsWith('.png') || cell.label.endsWith('.jpg'))) {
                            return minRowHeight;
                        }
                        // เผื่อ Padding ด้านข้าง 6 (ข้างละ 3) และบวก Padding บน-ล่าง เพิ่ม 10
                ...
                // 4. ตรวจสอบการขึ้นหน้าใหม่
                if (rowY + textHeight > bottomLimit) {
                    doc.addPage();
                    pageNo++;
                    rowY = drawVisitorHeader(doc);
                    drawPageNumberTopRight(doc, pageNo);
                    doc.fontSize(currentFontSize); // รีเซ็ต Font หลังขึ้นหน้าใหม่
                }

                // 5. วาดเนื้อหาลงในแต่ละ Cell
                let currentX = mLeft;
                cells.forEach(cell => {
                    // คำนวณหาความสูงของข้อความจริงในช่องนี้ เพื่อหาจุดเริ่มเขียนให้กึ่งกลาง (Vertical Center)
                    const actualTextHeight = doc.heightOfString(String(cell.label || ''), {
                        width: cell.width - 6,
                        lineGap: lineGapValue
                    });

                    // สูตรคำนวณตำแหน่ง Y: จุดเริ่มแถว + (ส่วนต่างความสูงที่เหลือ / 2)
                    const individualValueY = rowY + (textHeight - actualTextHeight) / 2;

                    drawCell(doc, cell, currentX, rowY, textHeight, individualValueY, lineGapValue);
                    currentX += cell.width;
                });

                // 6. วาดเส้นปิดขอบขวาสุด และเส้นแนวนอนปิดท้ายแถว
                ...
                // เลื่อนตำแหน่งแถวถัดไปลงมาตามความสูงจริงที่ใช้
                rowY += textHeight;
            }
        }

    } catch (err) {
        console.error("Error generating log book:", err);
    }
}


function drawCell(doc, cell, x, rowY, rowHeight, valueY, lineGapValue) {
    ...
    if (typeof label === 'string' && (label.endsWith('.png') || label.endsWith('.jpg'))) {
        ...
        if (fs.existsSync(imagePath)) {
            const imgHeight = 20; 
            const imgY = rowY + (rowHeight - imgHeight) / 2; // จัดกลางแนวตั้งสำหรับรูปภาพ
            doc.image(imagePath, contentX, imgY, {
                width: contentWidth,
                height: imgHeight,
                align: 'center'
            });
        ...
        listDataE.forEach(item => {
            item.people.forEach(person => {
                const cells= [
                    ...
                    { label: formatThaiDate(item.datetime_out), width: 55 },
                    { label: getTime(item.datetime_out), width: 50 },
                    ...
                    { label: getTime(item.datetime_in), width: 50 },
...
function drawTemporaryHeader(doc) {
    ...
    const header1 = [
        ...
        { text: 'วันที่คืน', w: 60 },
        { text: '  ลายมือชื่อ   ผู้คืนบัตร', w: 80, dh: true  },
