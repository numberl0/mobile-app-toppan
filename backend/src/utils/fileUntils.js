...
function cleanUpFolder(folderPath, uploadedFilenames) {
  ...
  const currentFiles = Array.isArray(uploadedFilenames) ? uploadedFilenames : [];
  // อ่านไฟล์ทั้งหมดที่มีอยู่ใน Folder บน Server ปัจจุบัน
  const filesInFolder = fs.readdirSync(folderPath);
  filesInFolder.forEach(file => {
    // ถ้าไฟล์ใน Folder "ไม่อยู่ใน" รายชื่อไฟล์ใหม่ที่ส่งมาล่าสุด -> ลบทิ้งทันที
...
const transformFilenameToUrlDoc = (data) =>
  data.map(record => {
    ...
    switch (record.request_type) {
      case 'VISITOR':
        dateFolder = record.datetime_in;
        pk = record.tno_pass;
        break;
      case 'EMPLOYEE':
        dateFolder = record.datetime_out;
        pk = record.tno_pass;
        break;
      case 'PERMISSION':
